...
class DaoAppointmentsPostgres implements DaoAppointments{
  ...
  Future<Either<Failure,bool>> addAppointment(Map<String, dynamic> appt) async{
    try{
      ...
      await conn.runTx((c)async{
        Result user = await c.execute(r'SELECT user_id FROM USERS WHERE cedula = $1', parameters: [appt["cedula"]]);
        if(user.isEmpty) {
          user =  await c.execute(
            r'INSERT INTO USERS(first_name, last_name, age, cedula) VALUES($1, $2, $3, $4) RETURNING user_id', 
            parameters: [appt["nombres"], appt["apellidos"], appt["edad"], appt["cedula"]]);
        }
        DateTime date = appt["fecha"];
        TimeOfDay time = appt["hora"];
        String timestamp = DateTime(date.year, date.month, date.day, time.hour, time.minute).toIso8601String();
        await c.execute(r'INSERT INTO APPOINTMENTS(appointment_date, user_id) VALUES($1, $2)', parameters: [timestamp, user[0][0]]);
      });
  ...
  Future<Either<Failure,List<Map<String, dynamic>>>> seekAppointments(DateTime date) async {
    try{
      ...
      Result res = await conn.execute(Sql.named('SELECT * FROM USERS U INNER JOIN APPOINTMENTS A ON U.USER_ID = A.USER_ID WHERE A.appointment_date::DATE =  @date'), parameters: {'date': formatedDate});
      ...
      for(final arr in res){
        ...
        print(arr[6].toString());
        appt["hora"] = arr[6].toString().substring(11,13) + arr[6].toString().substring(14,16);
        response.add(appt);
        print(arr[0]); 
        print(arr[1]);
        print(arr[2]);
